...
export default function useTransactions() {
    ...
    const { data: transactions, error: organizationError, isLoading: organizationLoading } = useSWR(
        swrKey,
    ...
    const currentTransactions = transactions ? transactions.slice(
        (currentPage - 1) * transactionsPerPage,
        currentPage * transactionsPerPage
    ) : [];

    const sent = transactions ? transactions.filter(
        (transaction) => transaction.transactionType === "Disbursement Transaction"
    ) : [];

    const deposit = transactions ? transactions.filter(
        (transaction) => transaction.transactionType === "Collection Transaction"
    ) : [];

    const success = transactions ? transactions.filter(
        (transaction) => transaction.status === "SUCCESSFUL"
    ) : [];

    const failed = transactions ? transactions.filter(
        (transaction) => transaction.status !== "SUCCESSFUL"
    ) : [];
